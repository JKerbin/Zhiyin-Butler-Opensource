...
由于精力有限，目前作者在参加其他开发项目以及完成课业，该项目的开发维护暂时无限期停滞。本项目未来的计划包括参考copilot的功能模式，使用更先进的模型，以及新增基于springboot的服务端支持。同时客户端也面临着重构，不再使用electron可视化界面套壳模式，而是采用前后端分离的模式开发完整的客户端程序。
